...
public final class TitleScreen {
    ...
    public enum Action {
        NONE, NEW_GAME, LOAD_GAME, OPTIONS, QUIT
    }
    ...
    public Action update(Game g) {
        ...
        String[] labels = {"NEW FRONTIER", "LOAD FRONTIER", "GRAPHICS", "QUIT"};
        Action[] actions = {Action.NEW_GAME, Action.LOAD_GAME, Action.OPTIONS, Action.QUIT};
        float bw = Math.min(360, w * 0.52f), bh = 42;
        float x = w / 2f - bw / 2f, y0 = h * 0.49f;
        ...
        for (int i = 0; i < labels.length; i++) {
            ...
            ui.textCentered(w / 2f, y + 12, 1.5f, labels[i],
        ...
        if (!notice.isBlank()) {
            ui.textCentered(w / 2f, y0 + 214, 1.2f, notice, 1f, 0.65f, 0.42f, 1f);
        }
        ui.textCentered(w / 2f, h - 34, 1.05f,
                "N / L / O / Q     Enter selects     Esc quits",
