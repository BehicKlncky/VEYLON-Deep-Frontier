...
**v2** — saves from the old prototype are not loadable; start a new world.
...
.\gradlew.bat fatJar            # build\libs\veylon-0.2.0-all.jar
.\gradlew.bat jpackage          # build\jpackage\Veylon\Veylon.exe
.\gradlew.bat appImageZip       # build\distributions\veylon-0.2.0-windows-x64.zip
...
./gradlew appImageZip       # build/distributions/veylon-0.2.0-macos-{x64|arm64}.zip
./gradlew releaseArtifacts
```

The tag-triggered [release build workflow](.github/workflows/release-build.yml) creates
three self-contained archives: Windows x64, macOS Apple Silicon and macOS Intel. The
macOS archives are currently unsigned and not notarized; macOS may therefore require
an explicit approval in **System Settings → Privacy & Security** on first launch.

Automated OpenGL smoke test (opens a window and auto-exits; exercises save/load
v2, equipment, fire and storm):
...
```powershell
java --enable-native-access=ALL-UNNAMED -Xmx2G -jar build\libs\veylon-0.2.0-all.jar
```
...
com.veylon
  ...
  ui/                         - title/options/presentation, Hud, Inventory/Crafting/
                                Crate/Npc/Map screens, PauseMenu, DebugOverlay,
                                SimulationPanel, EventLog
